...
const TYPE_COLORS = {
  Ansiedade:   '#E05252',   // vermelho mais sóbrio
  Hábito:      '#D4783A',   // terracota/burnt sienna
  Cansaço:     '#7B68C8',   // roxo mais acinzentado
  Social:      '#3B82F6',   // azul limpo
  Tédio:       '#7A8494',   // cinza-azulado
  Estresse:    '#C95F8E',   // rosa queimado
  // Horário do dia — para o gráfico de cigarros
  'Manhã':     '#F59E0B',   // âmbar
  'Tarde':     '#D4783A',   // terracota
  'Noite':     '#7B68C8',   // roxo
  'Madrugada': '#3B82F6',   // azul
};
...
export const TriggersScreen = () => {
  ...
  // Recarrega quando um novo craving é registrado
  React.useEffect(() => {
  ...
  const handleEdit = (id) => {
    // TODO: requires backend PATCH /events/:id endpoint
  };
  ...
  // Distribuição por hora do dia para cigarros
            ...
            return (
              <div key={e.id} className={`triggers__list-row ${typeClass}`}>
                ...
                <div className="triggers__list-actions">
                  <button className="triggers__list-action-btn triggers__list-edit" onClick={() => handleEdit(e.id)} title="Editar">
                    <Icon name="Pencil" size={13} color="var(--color-muted)" />
                  </button>
